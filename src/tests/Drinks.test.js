...
const END_POINT_ALL = 'https://www.thecocktaildb.com/api/json/v1/1/search.php?s=';
const SEARCH_ICON = 'search icon';
const initialEntries = ['/drinks'];
const { searchResults: { drinks } } = initialState;

describe('1 - Drinks page, testing components render', () => {
  ...
  it('checks if Categories options are rendered as expected', async () => {
    ...
    renderWithRouterRedux(<Drinks />, { initialEntries, initialState });

    expect(global.fetch).toHaveBeenCalledTimes(2);
    expect(fetch).toHaveBeenNthCalledWith(1, 'https://www.thecocktaildb.com/api/json/v1/1/list.php?c=list');
...
describe('2 - Drinks page, testing Header and Footer buttons behavior', () => {
  afterEach(() => jest.restoreAllMocks());
...
describe('3 - Drinks page, CategoriesOptions buttons behavior', () => {
  afterEach(() => jest.restoreAllMocks());
  ...
  and category buttons overlap each other fetch when clicked`, async () => {
    ...
    expect(fetch).toBeCalledWith('https://www.thecocktaildb.com/api/json/v1/1/filter.php?c=Cocktail');
  });
...
describe('4 - Drinks page, SearchBar buttons behavior', () => {
  afterEach(() => jest.restoreAllMocks());
  ...
  it('checks if SearchBar Name button fetch by name searched', async () => {
    ...
    userEvent.click(searchBtn);
    expect(fetch).toBeCalledWith('https://www.thecocktaildb.com/api/json/v1/1/search.php?s=cocktail');
  });
...
describe('5 - Drinks page, CardRecipes behavior', () => {
  it('checks if Recipe card redirect to recipe details page on click', async () => {
    ...
    const corbaCard = screen.getByRole('img', { name: 'GG' });
